...
export default async function QuotationDetailPage({ params }: Props) {
  ...
  if (id === 'new') {
    const counter = (settings?.quote_counter ?? 0) + 1;
    const number = `${settings?.quote_prefix ?? 'QUO'}-${String(counter).padStart(4, '0')}`;
    return (
      <div className="page">
        <div className="page-header">
          <h1 className="page-title">New Quotation</h1>
          <Link href="/quotations" className="btn btn-ghost">← Back</Link>
        </div>
        <DocEditor
          doc={null} kind="quotation" defaultNumber={number}
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1 className="page-title">Quotation #{d.number}</h1>
